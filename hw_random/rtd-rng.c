...
static int rtd_rng_read(struct hwrng *rng, void *buf, size_t max, bool wait)
{
	void __iomem *rng_base = (void __iomem *) rng->priv;
	unsigned int tocnt = 0;

	/* Loop waiting for random data to be available */
	while (! (__raw_readl(rng_base + RNG_RETURN3) & 0x1) ) {
		if (!wait || tocnt++ > MAX_1MS_TO_CNT) {
			pr_err("%s: timeout waiting for data\n", __func__);
			return 0;
		}
		cpu_relax();
	}
...
static int rtd_rng_probe(struct platform_device *pdev)
{
	...
	if (!rng_base) {
		dev_err(dev, "failed to remap RNG registers");
		return -ENODEV;
	}
	...
	if (err) {
		dev_err(dev, "failed to register %s RNG\n", RNG_NAME);
		iounmap(rng_base);
		rtd_rng_ops.priv = (unsigned long) 0;
	} else {
		dev_info(dev, "registered %s RNG\n", RNG_NAME);
	}

	return err;
}

static void rtd_rng_remove(struct platform_device *pdev)
{
