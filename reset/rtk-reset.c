...
// Reset controller
////////////////////////////////////////////////////////////////////////////////
#define to_reset_priv(_p) container_of((_p), struct reset_priv, rc_dev)
...
static int rtk_reset_assert(struct reset_controller_dev *rc_dev,
		unsigned long id)
{
	struct reset_priv *priv = to_reset_priv(rc_dev);
	u32 val, mask;
...
static int rtk_reset_deassert(struct reset_controller_dev *rc_dev,
		unsigned long id)
{
	struct reset_priv *priv = to_reset_priv(rc_dev);
	u32 val, mask;
...
static int rtk_reset_reset(struct reset_controller_dev *rc_dev,
		unsigned long id)
{
	int ret;

	ret = rtk_reset_assert(rc_dev, id);
	if (ret)
		return ret;

	return rtk_reset_deassert(rc_dev, id);
}

static int rtk_reset_status(struct reset_controller_dev *rc_dev,
		unsigned long id)
{
	struct reset_priv *priv = to_reset_priv(rc_dev);
	unsigned int val = 0;
...
static int rtk_reset_of_xlate(struct reset_controller_dev *rc_dev,
...
static int rtk_reset_probe(struct platform_device *pdev)
{
	...
	priv->rc_dev.of_reset_n_cells = 1;
	priv->rc_dev.of_xlate = rtk_reset_of_xlate;
...
MODULE_DEVICE_TABLE(of, rtk_reset_match);
module_platform_driver(rtk_reset_driver);

MODULE_AUTHOR("Cheng-Yu Lee <cylee12@realtek.com>");
MODULE_DESCRIPTION("RTD129x reset controller");
