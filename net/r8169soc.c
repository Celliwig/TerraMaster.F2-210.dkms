...
#define CONFIG_ARCH_RTD129x						// Hardwire to Realtek RTD129x
...
//	struct device *device;
	...
	int (*get_link_ksettings)(struct net_device *, struct ethtool_link_ksettings *cmd);
...
static void r8169soc_ocp_reg_write(struct rtl8169_private *tp,
...
static unsigned int r8169soc_ocp_reg_read(struct rtl8169_private *tp,
				unsigned int reg_addr)
{
	void __iomem *ioaddr = tp->mmio_addr;
	unsigned int wdata;
...
static void r8169_mmd_write(struct rtl8169_private *tp, unsigned int dev_addr, unsigned int reg_addr, unsigned int value)
{
	// page 0
...
static unsigned int r8169_mmd_read(struct rtl8169_private *tp, unsigned int dev_addr, unsigned int reg_addr)
{
	// page 0
	r8169_mdio_write(tp, 0x1f, 0);
	// address mode
...
static void r8169_display_eee_info(struct net_device *dev, struct rtl8169_private *tp)
{
				...
				struct ethtool_drvinfo *info)
{
	...
	strscpy(info->bus_info, "RTK119X-ETN", sizeof(info->bus_info));
...
static int rtl8169_set_link_ksettings(struct net_device *dev, const struct ethtool_link_ksettings *cmd)
{
	struct rtl8169_private *tp = netdev_priv(dev);
	int ret;
	u32 advertising = 0;
...
static int rtl8169_gset_tbi(struct net_device *dev, struct ethtool_link_ksettings *cmd)
{
	struct rtl8169_private *tp = netdev_priv(dev);
	void __iomem *ioaddr = tp->mmio_addr;
	u32 status;
...
static int rtl8169_gset_xmii(struct net_device *dev, struct ethtool_link_ksettings *cmd)
{
	struct rtl8169_private *tp = netdev_priv(dev);
...
static int rtl8169_get_link_ksettings(struct net_device *dev, struct ethtool_link_ksettings *cmd)
{
	struct rtl8169_private *tp = netdev_priv(dev);
	int rc;
...
static bool rtl_fw_format_ok(struct rtl8169_private *tp, struct rtl_fw *rtl_fw)
{
	...
	if (!fw_info->magic) {
		...
		strscpy(version, rtl_lookup_firmware_name(tp), RTL_VER_SIZE);
...
static int rtl_set_mac_address(struct net_device *dev, void *p)
{
	...
	memcpy((void *) dev->dev_addr, addr->sa_data, dev->addr_len);
...
static void r8169soc_reset_phy_gmac(struct rtl8169_private *tp)
{
	...
	if (tp->output_mode != OUTPUT_EMBEDDED_PHY) {
		...
		rstc_sds_reg = reset_control_get_shared(&tp->pdev->dev, "sds_reg");
		rstc_sds = reset_control_get_shared(&tp->pdev->dev, "sds");
		rstc_pcie0_power =
			reset_control_get_shared(&tp->pdev->dev, "pcie0_power");
		rstc_pcie0_phy =
			reset_control_get_shared(&tp->pdev->dev, "pcie0_phy");
		rstc_pcie0_sgmii_mdio =
			reset_control_get_shared(&tp->pdev->dev, "pcie0_sgmii_mdio");
		rstc_pcie0_phy_mdio =
			reset_control_get_shared(&tp->pdev->dev, "pcie0_phy_mdio");
...
static u32 r8169soc_serdes_init(struct rtl8169_private *tp)
{
	...
	struct reset_control *rstc_sds_reg =
		reset_control_get_shared(&tp->pdev->dev, "sds_reg");
	struct reset_control *rstc_sds =
		reset_control_get_shared(&tp->pdev->dev, "sds");
	struct reset_control *rstc_pcie0_power =
		reset_control_get_shared(&tp->pdev->dev, "pcie0_power");
	struct reset_control *rstc_pcie0_phy =
		reset_control_get_shared(&tp->pdev->dev, "pcie0_phy");
...
static void r8169soc_reset_phy_gmac(struct rtl8169_private *tp)
{
	...
	rstc_sds_reg = reset_control_get_shared(&tp->pdev->dev, "sds_reg");
	rstc_sds = reset_control_get_shared(&tp->pdev->dev, "sds");
	rstc_pcie0_power = reset_control_get_shared(&tp->pdev->dev, "pcie0_power");
	rstc_pcie0_phy = reset_control_get_shared(&tp->pdev->dev, "pcie0_phy");
	rstc_pcie0_sgmii_mdio = reset_control_get_shared(&tp->pdev->dev, "pcie0_sgmii_mdio");
	rstc_pcie0_phy_mdio = reset_control_get_shared(&tp->pdev->dev, "pcie0_phy_mdio");
...
//	pci_set_master(pdev);
	...
	if (rtl_tbi_enabled(tp)) {
		...
		tp->get_link_ksettings = rtl8169_gset_tbi;
		...
		tp->get_link_ksettings = rtl8169_gset_xmii;
	...
	/* Get MAC address */
	rc = of_get_mac_address(pdev->dev.of_node, mac_addr);
	if (!rc)
	...
	for (retry = 0; retry < RETRY_MAX; retry++) {
		for (i = 0; i < ETH_ALEN; i++)
			*(((unsigned char *) ndev->dev_addr) + i) = RTL_R8(MAC0 + i);
