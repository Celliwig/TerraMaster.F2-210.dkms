...
static int dw8250_handle_irq(struct uart_port *p)
{
	...
	if (serial8250_handle_irq(p, iir)) {
		if (d->isr_reg) {
                        writel(d->isr_status_mask, d->isr_reg);
		}
...
static int dw8250_probe(struct platform_device *pdev)
{
	...
	isr_regs = platform_get_resource(pdev, IORESOURCE_MEM, 1);
	if (!isr_regs)
		return dev_err_probe(dev, -EINVAL, "no ISR registers defined\n");
	...
	if (!device_property_read_u32(dev, "interrupts-st-mask", &data->isr_status_mask))
...
static const struct dw8250_platform_data dw8250_realtek_129x_data = {
	.usr_reg = DW_UART_USR,
	.quirks = DW_UART_QUIRK_REALTEK,
};

static const struct of_device_id dw8250_of_match[] = {
	{ .compatible = "rtk,dw-apb-uart", .data = &dw8250_realtek_129x_data },
	{ /* Sentinel */ }
};
