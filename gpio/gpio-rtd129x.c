 ...
 * Fix set_default_gpio / power management
 */
...
static int gpio_chk_irq_enable(struct rtk_gpio_controller *p_rtk_gpio_ctl, u32 irq)
{
	u32 ie_offset;
...
static void gpio_irq_handler(struct irq_desc *desc)
{
	...
	if (!irq_had_fired) {
		RTK_GPIO_DBG("%s gpio irq(hwirq=%d) has triggered but no dispatch", __func__, hw_irq);
...
static int rtk_gpio_irq_setup(struct device_node *node,
	struct rtk_gpio_controller *p_rtk_gpio_ctl,
...
//static void set_default_gpio(struct device_node *node)
//{
//	int num_gpios, n;
//
...
static int rtk_gpio_set_config(struct gpio_chip *chip, unsigned offset, unsigned long config)
{
	u32 debounce;
	int ret = -ENOTSUPP;
	...
	case PIN_CONFIG_INPUT_DEBOUNCE:
		...
		ret = rtk_gpio_setdeb(chip, offset, debounce);
		break;
	default:
		break;
	}
...
static int rtk_gpio_probe(struct platform_device *pdev)
{
	...
	pr_info("[%s] initialising GPIO controller\n", DEV_NAME);
	...
	p_rtk_gpio_ctl->chip.set_config = rtk_gpio_set_config;
	...
	if (gpiochip_add_data(&p_rtk_gpio_ctl->chip, NULL))
		pr_info("[%s]  %s  line: %d\n", __FILE__, __func__, __LINE__);
...
//
//	if (!node) {
//		pr_err("rtk gpio: error, OF node not available.\n");
//		return -ENODEV;
//	}
//
//	p_rtk_gpio_ctl = platform_get_drvdata(pdev);
...
MODULE_DEVICE_TABLE(of, rtk_gpio_of_match);
module_platform_driver(rtk_gpio_driver);

MODULE_AUTHOR("Realtek Semiconductor Corporation");
MODULE_DESCRIPTION("Realtek RTD129x GPIO controller");
