...
static int RTK_pinctrl_suspend(struct platform_device *pdev, pm_message_t state)
{
	struct RTK_pinctrl *pctl;
	void __iomem *addr;
	unsigned int i;
...
static int RTK_pinctrl_resume(struct platform_device *pdev)
{
	struct RTK_pinctrl *pctl;
	void __iomem *addr;
	unsigned int i;
...
MODULE_DEVICE_TABLE(of, RTK_pinctrl_match);
module_platform_driver(RTK_pinctrl_driver);

MODULE_AUTHOR("Realtek Semiconductor Corporation");
MODULE_DESCRIPTION("Realtek RTD129x pinctrl controller");
